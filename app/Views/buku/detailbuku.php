                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-none d-md-block">
                            ...
                            <p>Ketersediaan Buku : <strong class="text-success"><?= $buku['stok'] ?></strong></p>
                        </div>

                        <div class="my-2 row px-3 gap-2 flex-nowrap">
                            <a href="#" class="col-2 p-1 btn text-success border border-success border-2 bookmark" data-bs-toggle="tooltip" data-bs-title="Tambahkan Ke Koleksi"><i class="fa-regular fa-bookmark"></i></a>
                            <?php if (logged_in()) : ?>
                                <?php if (isset($terpinjam)) : ?>
                                    <div class="col btn-group p-0" role="group" aria-label="Basic example">
                                        <a class=" btn btn-success d-flex justify-content-center gap-2 align-items-center fs-6">Terpinjam</a>
                                        <a class=" btn border-success border-2 d-flex justify-content-center align-items-center fs-6 p-0" href="/pinjam" data-bs-toggle="tooltip" data-bs-title="Lihat Pinjaman"><i class="fa-solid fa-eye text-success"></i></a>
                                    </div>
                                <?php else : ?>
                                    <a class="col btn btn-success d-flex justify-content-center align-items-center fs-6" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#pinjam">Pinjam Sekaramg</a>
                                <?php endif ?>

                            <?php else : ?>
                                <a href="/login" class="col btn btn-success d-flex justify-content-center align-items-center fs-6" class="btn btn-primary">Pinjam langsung</a>
                            <?php endif ?>
                        </div>
            ...
            <form method="post" action="/pinjam/<?= $buku['slug'] ?>" class="modal-content">
                ...
                <div class="modal-body row">
                    ...
                    <div class="col-12 col-md-8">
                        ...
                        <p><strong class="<?= $batas_pinjam == 0 ? 'text-danger' : '' ?>"><?= $batas_pinjam  ?></strong> Buku</p>
                        <p class="small m-0">Catatan :</p>
                        <ul>
                            <li class="small">Kembalikan buku keperpustakaan untuk menambah batas peminjaman</li>
                            <li class="small">Pengambilan buku ke perpustakaan berlaku 2 hari setelah mengklik tombol pinjam</li>
                            <li class="small">Batas peminjaman buku 7 hari setelah buku di ambil dari perpustakaan</li>
                        </ul>
                    </div>
                </div>
                <div class="modal-footer">
                    <p class="me-auto text-danger"><?= session()->getFlashdata('error_pinjam') ? session()->getFlashdata('error_pinjam') : '' ?></p>
